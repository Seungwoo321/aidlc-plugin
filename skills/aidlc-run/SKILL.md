---
name: aidlc-run
description: AIDLC 볼트 사이클 실행. aidlc-harness가 생성한 도메인 에이전트와 plan.md를 기반으로 볼트 사이클을 실행합니다. 실행 모드(에이전트 팀/서브 에이전트)는 harness-report.md에 따라 결정. Use when user mentions "볼트 실행", "실행", "aidlc 실행", "run", or asks to "start bolts", "run bolts", "실행 시작", "개발 시작".
user-invocable: true
...
하네스 산출물(에이전트, plan.md, harness-report.md)을 기반으로 볼트 사이클을 실행합니다.
harness-report.md의 실행 모드에 따라 에이전트 팀(TeamCreate) 또는 서브 에이전트(Agent 직접 호출)로 동작합니다.
본 에이전트(스킬 실행자)가 Team Lead / Orchestrator 역할을 수행합니다.
...
- `docs/aidlc-docs_*/plan.md` 존재 (작업 계획)
- `docs/aidlc-docs_*/harness-report.md` 존재 (하네스 구성)
- `.claude/agents/` 에 도메인 에이전트 파일 존재

## 세션 이어가기

이전 세션에서 팀이 작업하다 종료된 경우, plan.md에 기록된 진행 상태를 기반으로 이어갑니다.
- plan.md의 `[x] @{agent-name}` 체크된 항목은 완료로 간주 — 태그로 누가 완료했는지 즉시 파악
- 미완료(`[ ] @{agent-name}`) 항목부터 태스크를 생성 — 태그로 담당 에이전트 확인
...
3. **현재 프로젝트 단계를 판단합니다:**
   ...
   | 단계 | 판단 기준 |
   |------|----------|
   | **Inception** | 화면기획/스키마 정의 등 기획 태스크가 미완료 |
...
harness-report.md의 에이전트 구성, 오케스트레이션 설계(데이터 흐름, Phase별 팀원 구성, 에러 핸들링), 현재 볼트 특성에 따라 에이전트를 선별한다.
...
harness-report.md의 `실행 모드` 필드를 확인하여 분기한다.

#### A. 에이전트 팀 모드 (에이전트 간 통신 필요)
...
팀원들은 SendMessage로 직접 통신하고, 공유 작업 목록(TaskCreate/TaskUpdate)으로 자체 조율한다.

#### B. 서브 에이전트 모드 (에이전트 간 통신 불필요)

TeamCreate를 사용하지 않는다. Team Lead가 Agent 도구로 서브 에이전트를 직접 호출하고, 결과를 수집하여 통합한다. 서브 에이전트끼리는 통신하지 않는다.

### 4단계: 태스크 생성

plan.md의 미완료 항목을 기반으로 태스크를 생성합니다.

**에이전트 태그 활용:** plan.md의 각 태스크에는 `@{agent-name}` 태그가 있다. 이 태그를 파싱하여 TaskCreate 시 해당 에이전트에 할당한다.

```
plan.md: - [ ] `@frontend-dev` Header 컴포넌트 구현
  → TaskCreate: assignee=frontend-dev, description="Header 컴포넌트 구현"
```
...
각 에이전트의 프롬프트에는 반드시 다음을 포함합니다:
- 프로젝트 경로 (cwd)
- AIDLC 문서 경로
- 프로젝트 CLAUDE.md 내용 요약
- **에이전트 팀 모드인 경우:** 팀 이름 (team_name), 다른 팀원 이름 (P2P 통신용)
- **서브 에이전트 모드인 경우:** team_name 생략, 결과를 Team Lead에게만 반환하도록 지시
...
`.claude/agents/{name}.md`에 정의된 커스텀 에이전트는 `subagent_type`으로 직접 사용할 수 없다. 반드시 `subagent_type: "general-purpose"`로 소환하고, 프롬프트에서 에이전트 정의 파일을 읽도록 지시한다.
...
Agent:
  ...
  run_in_background: true
  prompt: |
    먼저 .claude/agents/{agent-name}.md를 읽고 그 역할과 원칙에 따라 동작하세요.
...
Agent:
  ...
  prompt: |
    ...
    TaskList에서 자신에게 할당된 태스크를 확인하세요.
    완료 후 결과를 반환하세요. (다른 에이전트와 직접 통신하지 않습니다)
```
...
3. **볼트 전환 시 재구성**
   ...
   - 이전 산출물을 파일로 저장하여 보존한다
   - **에이전트 팀 모드:** 팀원에게 shutdown 메시지 → TeamDelete → 새 TeamCreate → 에이전트 소환
   - **서브 에이전트 모드:** 이전 에이전트는 자동 종료됨. 새 볼트에 맞는 에이전트를 Agent 도구로 소환
   - 사용자에게 재구성을 보고한다
...
7. **종료 (모든 볼트 완료 시)**
   - 모든 태스크 완료 확인
   - plan.md 최종 동기화
   - **에이전트 팀 모드:** TeamDelete로 팀을 정리한다
   - **서브 에이전트 모드:** 정리 불필요 (자동 종료)
   - 최종 결과를 사용자에게 보고한다
