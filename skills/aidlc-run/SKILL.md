---
name: aidlc-run
description: AIDLC 볼트 사이클 실행. aidlc-harness가 생성한 도메인 에이전트와 plan.md를 기반으로 팀을 구성하고 볼트 사이클을 실행합니다. Use when user mentions "볼트 실행", "실행", "aidlc 실행", "run", or asks to "start bolts", "run bolts", "실행 시작", "개발 시작".
user-invocable: true
...
하네스 산출물(에이전트, plan.md)을 기반으로 에이전트 팀을 구성하고 볼트 사이클을 실행합니다.
...
3. **볼트 전환 시 팀 재구성**
   ...
   - 이전 팀의 산출물을 파일로 저장하여 보존한다
   - 필요 없어진 에이전트에게 shutdown 메시지를 보낸다
   - TeamDelete로 이전 팀을 정리한다
   - 새 팀을 TeamCreate로 구성하고 필요한 에이전트를 소환한다
   - 사용자에게 팀 재구성을 보고한다
