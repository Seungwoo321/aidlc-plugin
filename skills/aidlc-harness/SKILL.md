---
name: aidlc-harness
description: AIDLC 프로젝트의 설계와 구성을 담당하는 하네스. aidlc-init 산출물(보고서 2개)을 기반으로 작업 계획(plan.md), 폴더 구조, 도메인 에이전트, 스킬을 생성합니다. Use when user mentions "하네스", "harness", "에이전트 생성", "도메인 에이전트", or asks to "generate agents", "create harness", "에이전트 만들어줘", "하네스 구성". /aidlc-init 완료 후 실행.
user-invocable: true
...
aidlc-init 산출물을 기반으로 작업 계획, 폴더 구조, 도메인 에이전트와 스킬을 생성한다.

<role>
이 스킬은 AI-DLC 방법론의 두 번째 단계로, 프로젝트 보고서를 분석하여 작업 계획을 수립하고, 도메인 특화 에이전트 팀을 설계하고 생성한다.

핵심 원칙:
- aidlc-init의 보고서를 입력으로 사용하여 중복 질문을 하지 않는다
- 작업 계획(plan.md)과 폴더 구조를 먼저 생성한 후 에이전트를 설계한다
...
2. Phase 1: 도메인 분석
   ↓
3. Phase 2: 작업 계획 및 폴더 구조 생성
   → 사용자 승인
   ↓
4. Phase 3: 팀 아키텍처 설계
   → 사용자 승인
   ↓
5. Phase 4: 에이전트 정의 생성
   → 사용자 검토
   ↓
...
## Phase 2: 작업 계획 및 폴더 구조 생성

### 2-1. 작업 계획(plan.md) 생성

보고서의 설계 방법론, 아키텍처 패턴, AI-DLC 적용 계획을 바탕으로 작업 계획을 생성한다.

`docs/aidlc-docs_{주제}/plan.md`에 다음을 포함한다:
- AI-DLC 단계별(Inception, Construction, Operations) 태스크
- 각 태스크는 체크박스(`[ ]`)로 표시하여 진행 추적 가능
- 태스크 간 의존성과 순서를 명확히 표시
- 볼트 단위로 그룹핑
...
AskUserQuestion으로 작업 계획을 제안한다:
```
작업 계획(plan.md)을 생성했습니다.
...
(서브 에이전트 모드에서는 이 섹션 생략 가능 — 결과를 리더에게만 반환)
...
- **`subagent_type`은 항상 `"general-purpose"`**: 커스텀 에이전트는 subagent_type으로 직접 사용 불가. 프롬프트에서 `.claude/agents/{name}.md`를 읽도록 지시
...
Phase 3에서 선택한 실행 모드에 맞는 템플릿(A: 에이전트 팀 / B: 서브 에이전트)을 기반으로, aidlc-run이 생성된 에이전트를 조율할 수 있도록 설정한다.

### 6-1. plan.md에 에이전트 태그 부여

Phase 4에서 에이전트가 확정되었으므로, plan.md의 각 태스크에 `@{agent-name}` 태그를 부여한다. 이 태그는 aidlc-run이 태스크를 에이전트에 할당하는 근거가 된다.

**태그 형식:** `- [ ] @{agent-name} {태스크 설명}`

예시:
```markdown
## Construction - Bolt 1: 핵심 컴포넌트 구현
- [ ] `@frontend-dev` Header 컴포넌트 구현
- [ ] `@frontend-dev` Sidebar 컴포넌트 구현
- [ ] `@api-dev` /api/auth 엔드포인트 구현
- [ ] `@qa-inspector` Header + Sidebar + API 통합 테스트
```

규칙:
- 모든 태스크에 정확히 하나의 에이전트 태그를 부여한다
- 태그명은 `.claude/agents/{name}.md`의 name과 일치해야 한다
- 여러 에이전트가 협업하는 태스크는 주 담당자를 태그하고, 협업 에이전트는 태스크 설명에 명시한다
...
- [ ] plan.md의 모든 태스크에 `@{agent-name}` 태그가 부여됨
- [ ] 태그명이 `.claude/agents/` 내 에이전트 파일명과 일치
...
- [ ] aidlc-run이 생성된 에이전트를 스캔하여 팀을 구성할 수 있는 정보가 충분
</execution>

<constraints>
DO:
- aidlc-init 보고서를 입력으로 활용하여 중복 질문 최소화
- plan.md를 먼저 생성하고 에이전트 역할과 태스크를 정합시킨다
...
- Phase 2, Phase 3에서 사용자 승인 없이 다음 Phase로 넘어가지 않는다
</critical>
